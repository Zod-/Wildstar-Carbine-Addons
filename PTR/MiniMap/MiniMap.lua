...
function MiniMap:OnMatchMakerQueueBtn(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return
	end
...
function MiniMap:UpdateMatchMakerQueueBtn()
	self.wndMain:FindChild("MatchMakerQueueBtnIcon"):Show(MatchMakingLib.IsQueuedForMatching())
end

function MiniMap:OnRapidTransportOpen(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return
	end
