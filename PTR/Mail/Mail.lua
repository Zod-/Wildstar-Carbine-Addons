...
local kstrGiftMailIcon 	= "Icon_Windows_UI_NewGift"
...
function Mail:OnAvailableMail(arItems, bNewMail)
	...
	if bNewMail then
		Sound.Play(Sound.PlayUINewMailReceivedSFX)
	end
...
function Mail:OnMailResult(eResult)
	local tMailResultError =
	{
		...
		[GameLib.CodeEnumGenericError.Mail_LevelRequirement] 			= 	"GenericError_Mail_LevelRequirement",
		[GameLib.CodeEnumGenericError.Mail_CannotSendAttachments]		=	"GenericError_Mail_CannotSendAttachment",
	}
	...
	if tMailResultError[eResult] then
		...
		wndErrorMessageBodyText:SetAML("<P Font=\"CRB_InterfaceMedium\" TextColor=\"UI_TextHoloBody\">"..String_GetWeaselString(Apollo.GetString(tMailResultError[eResult])).."</P>")
...
function Mail:UpdateListItem(wndMailItem, msgMail)
	...
	if tMsgInfo.bIsGift then
		wndMailItem:FindChild("Icon"):SetSprite(kstrGiftMailIcon)
	elseif tMsgInfo.bIsRead then
	...
	if tMsgInfo.bNoExpiry then
		wndMailItem:FindChild("DateText"):SetText("")
	else
		local strExpires = ""
...
function Mail:OnMailItemClick(wndHandler, wndControl)
	...
	if tMessageInfo ~= nil then
		...
		if not tMessageInfo.bIsGift then
...
function MailCompose:OnQueryDragDrop(wndHandler, wndControl, nX, nY, wndSource, strType, nValue)
	...
	if strType == "DDBagItem" then
		...
		if self.tDragDrop.itemSource:IsAlwaysTradeable() or self.tDragDrop.itemSource:IsAccountTradeable() then
...
function MailCompose:OnDragDrop(wndHandler, wndControl, nX, nY, wndSource, strType, nValue)
	...
	if not self.tDragDrop.itemSource:IsAlwaysTradeable() and not self.tDragDrop.itemSource:IsAccountTradeable() then
		return
	end
...
function MailCompose:AppendAttachment(nValue)
	...
	if itemAttach and not itemAttach:IsAlwaysTradeable() and not itemAttach:IsAccountTradeable() then
...
function MailCompose:UpdateControls()
	...
	local bHasAccountAttachments = false
	for idx, wndIcon in pairs(self.arWndAttachmentIcon) do
		...
		if itemAttachment ~= nil then
			...
			bHasAccountAttachments = bHasAccountAttachments or itemAttachment:IsAccountTradeable()
			wndIcon:GetParent():FindChild("AccountBoundIndicator"):Show(bHasAccountAttachments)
		else
			...
			wndIcon:GetParent():FindChild("AccountBoundIndicator"):Show(false)
	...
	self.wndMain:FindChild("AccountBoundAlert"):Show(bHasAccountAttachments)
