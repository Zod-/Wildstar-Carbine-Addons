...
local ktUnlockActions =
{
	...
	{ strField = "nId", 	eValue = GameLib.LevelUpUnlock.Builder_AMPPoint_PerLevel, 					strEvent = "LevelUpUnlock_AMPPoint" },
...
function LevelUpUnlocks:OnSave(eType)
	...
	return
	{
		bSeenFirstReminder = self.bSeenFirstReminder
	}
end
...
function LevelUpUnlocks:OnRestore(eType, tSavedData)
	...
	if tSavedData.bSeenFirstReminder ~= nil then
		self.bSeenFirstReminder = tSavedData.bSeenFirstReminder
	end
end
...
function LevelUpUnlocks:OnCloseToReminder() -- Also from XML
	...
	if GameLib.IsTutorialZone(nZoneId) then
		return
	end
	
	self.wndLevelUpUnlockReminder = Apollo.LoadForm(self.xmlDoc, "LevelUpUnlockReminder", nil, self)
	
	self.wndLevelUpUnlockReminder:FindChild("TutorialCallout_ChooseCategory"):Show(not self.bSeenFirstReminder)
	self.bSeenFirstReminder = true
