...
function ArenaTeam:ResetRosterMemberButtons(bPreventClose)
	...
	local wndPromoteBtn = self.wndMain:FindChild("RosterOptionBtnPromote")
	local wndDemoteBtn = self.wndMain:FindChild("RosterOptionBtnDemote")
	local wndDisbandBtn = self.wndMain:FindChild("RosterDisbandBtn")
	local wndLeaveBtn = self.wndMain:FindChild("RosterLeaveBtn")
	local wndAddBtn = self.wndMain:FindChild("RosterOptionBtnAdd")
	local wndInviteBtn = self.wndMain:FindChild("RosterOptionBtnInvite")

	wndAddBtn:Show(false)
	wndAddBtn:FindChild("AddMemberContainer"):Show(false)
	wndRemoveBtn:Show(false)
	wndRemoveBtn:FindChild("RemoveMemberContainer"):Show(false)
	wndDisbandBtn:Show(false)
	wndDisbandBtn:SetCheck(false)
	...
	wndLeaveBtn:FindChild("LeaveContainer"):Show(false)
	wndLeaveBtn:Show(true)
	wndInviteBtn:Show(true)
	
	if not bPreventClose then
		wndAddBtn:FindChild("AddMemberContainer"):Close()
		wndRemoveBtn:FindChild("RemoveMemberContainer"):Close()
		wndDisbandBtn:FindChild("DisbandContainer"):Close()
		wndLeaveBtn:FindChild("LeaveContainer"):Close()
	end	
	...
	if guildCurr and guildCurr:GetMyRank() then
		...
		wndDemoteBtn:Enable(bSomeRowIsPicked and bTargetIsUnderMyRank and tSelected.nRank ~= 10) -- Can't go below 10
		wndDemoteBtn:Show(tMyRankPermissions and tMyRankPermissions.bChangeMemberRank)
				
		wndDisbandBtn:Show(guildCurr:GetMyRank() == 1)
		wndLeaveBtn:Show(guildCurr:GetMyRank() ~= 1)
	
		local bCanInvite = not GroupLib.InGroup() or GroupLib.AmILeader() or GroupLib.GetGroupMember(1).bCanInvite
		wndInviteBtn:Enable(bValidUnit and bCanInvite)
	end
	
	self.wndMain:FindChild("Controls"):ArrangeChildrenHorz(Window.CodeEnumArrangeOrigin.LeftOrTop)
end
...
function ArenaTeam:OnRosterDemoteMemberClick( wndHandler, wndControl, eMouseButton )
...
function ArenaTeam:OnRosterPromoteMemberCloseBtn( wndHandler, wndControl, eMouseButton )
	self.wndMain:FindChild("PromoteMemberContainer"):Show(false)
	self.wndMain:FindChild("RosterOptionBtnPromote"):SetCheck(false)
end

function ArenaTeam:OnRosterPromoteMemberClick( wndHandler, wndControl, eMouseButton )
	local guildCurr = self.wndMain:GetData()
	local tMember = self.wndMain:FindChild("RosterGrid"):GetData()
...
function ArenaTeam:OnRosterPromoteMemberYesClick( wndHandler, wndControl, eMouseButton )
	local guildCurr = self.wndMain:GetData()
	local tMember = self.wndMain:FindChild("RosterGrid"):GetData()
