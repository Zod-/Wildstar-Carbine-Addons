...
function ZoneMap:CreatePOIIcons()
	self.tPOITypes =
	{
		...
		[GameLib.CodeEnumMapOverlayType.TrackedUnit]	= {strSprite = "",												eCategory = nil,									strType = Apollo.GetString("ZoneMap_Event")},
...
function ZoneMap:BuildShownTypesArrays()
	...
	self.arAllowedTypesSuperPanning =
	{
		...
		GameLib.CodeEnumMapOverlayType.TrackedUnit
	}
...
function ZoneMap:OnDocumentReady()
	...
	local tButtonList =
	{
		...
		[ktMarkerCategories.CityDirections] 	= {strLabel = Apollo.GetString("ZoneMap_CityDirections"), 		bShown = false,	strIcon = "Icon_MapNode_Map_CityDirections"},
...
function ZoneMap:OnCharacterCreated()
	local tCurrentZoneMap = GameLib.GetCurrentZoneMap()
	if tCurrentZoneMap then
		self.wndZoneMap:SetZone(tCurrentZoneMap.id)
		self:OnMissionsCheck()
...
function ZoneMap:OnZoneMap_OpenMapToQuest(queArg)
	...
	if self.objActiveRegionUserData == nil or self.objActiveRegionUserData ~= queArg then
		...
		if tQuestLoc then
			...
			self.oShownLocObject = self.wndZoneMap:AddObject(self.eObjectTypeLocation, tQuestLoc.tIndicator, "", tInfo, {bNeverShowOnEdge = true})
		else
