...
function PvPKillBoard:OnPvpKillNotification(strVictimName, eReason, strKillerName, eKillerClass, eVictimTeam)
	...
	if eVictimTeam == MatchingGameLib.Team.Team1 then
		crKiller = kcrBlue
		crVictim = kcrRed
	else
		crKiller = kcrRed
...
function PvPKillBoard:OnPVPMatchEntered()
	if not MatchingGameLib.IsInPvpGame() then
		return
	end
