...
function TradeskillSchematics:FullRedraw(nSchematicIdToOpen)
	...
	for idx, tCurrData in ipairs(tTradeskills) do
		...
		if tCurrTradeskillInfo.bIsHobby then
			...
			wndTop:FindChild("TopLevelBtn"):SetText(tCurrTradeskill.strName)
		...
		elseif tCurrTradeskillInfo.bIsActive then
			...
			if tMiddleCategories then
				for nTierIdx = tCurrTradeskillInfo.eTier, 1, -1 do -- Start at current, then count down
					...
					wndTop:FindChild("TopLevelBtn"):SetText(tTier.strSubGroupName)
					wndTop:FindChild("TopLevelIcon"):SetSprite(kTradeskillIdToIcon[tCurrTradeskill.eId])
		...
		for idx2, tSchematic in pairs(tSchematicList) do
			...
			if bValidOneUse and (bHaveMaterials or not bFilterMaterials) then
				...
				if wndMiddle:FindChild("MiddleLevelBtn"):IsChecked() then
					...
					local nNumCraftable = nil
					local strNumCraftable = ""
					
					for key, tMaterial in pairs(tSchematicInfo.arMaterials) do
						if tMaterial.nNeeded > 0 then
							if nNumCraftable == nil then
								nNumCraftable = math.floor(tMaterial.nOwned / tMaterial.nNeeded)
							else
					...
					if nNumCraftable ~= nil and nNumCraftable > 0 then
						strNumCraftable = String_GetWeaselString(Apollo.GetString("CRB_BracketsNumber_Space"), nNumCraftable)
					end
					
					wndBottomItem:FindChild("BottomItemBtn"):SetData(tSchematic)
					wndBottomItem:FindChild("BottomItemBtn"):SetText(string.format("%s %s", tSchematic.strName, strNumCraftable))
...
function TradeskillSchematics:ResizeTree()
	...
	local nVScrollPosition = wndLeftSideScroll:GetVScrollPos()
			...
			for key2, wndMiddle in pairs(wndTop:FindChild("TopLevelItems"):GetChildren()) do
				if wndMiddle:FindChild("MiddleLevelBtn"):IsChecked() then
					for key3, wndBot in pairs(wndMiddle:FindChild("MiddleLevelItems"):GetChildren()) do
						...
						if Apollo.GetTextWidth("CRB_InterfaceMedium", wndBottomLevelBtn:GetText()) > wndBottomLevelBtn:GetWidth() - 10 then -- TODO QUICK HACK
	...
	wndLeftSideScroll:SetVScrollPos(nVScrollPosition or 0)
end
...
function TradeskillSchematics:DrawSchematic(tSchematic)
	...
	for key, tMaterial in pairs(tSchematicInfo.arMaterials) do
		if tMaterial.nNeeded > 0 then
			...
			local luaSubClass = wndMaterial:FindChild("MaterialsIcon"):GetWindowSubclass()
			luaSubClass:SetItem(tMaterial.itemMaterial)
	...
	local luaSubclass = wndSchem:FindChild("SchematicIcon"):GetWindowSubclass()
	
	wndSchem:Show(true)
	wndSchem:SetData(tSchematic)
	luaSubclass:SetItem(tSchematicInfo.itemOutput)
