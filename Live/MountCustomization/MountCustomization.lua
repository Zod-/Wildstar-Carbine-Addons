...
function MountScreen:OnShowUnknown(wndControl, wndHandler)
	for idx, wndMount in pairs(self.tUnknownMounts) do
		...
		local tSearchIndices = string.find(string.lower(wndMount:GetData().strName), string.lower(self.wndMain:FindChild("SearchField"):GetText()), 1, true)
...
function MountScreen:OnSearchFieldChanged(wndHandler, wndControl, strText)
	...
	for idx, wndMount in pairs(wndMountList:GetChildren()) do
		...
		if (tData.bIsKnown or self.bShowUnknown) and string.find(string.lower(tData.strName), strText, 1, true) then
			wndMount:Show(true)
		else
			wndMount:Show(false)
		end
