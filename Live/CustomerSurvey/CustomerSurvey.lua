...
function CustomerSurvey:UpdateAlert()
	...
	for idx = 1, 3 do
		local csCurSurvey = CustomerSurveyLib.GetPending(idx)
		if csCurSurvey and csCurSurvey:GetSurveyId() ~= CustomerSurveyLib.CodeEnumCustomerSurvey.LogOff then
			local wndAlert = Apollo.LoadForm("CustomerSurvey.xml", "CustomerSurveyAlert", self.wndAlertContainer:FindChild("List"), self)
			wndAlert:SetData(csCurSurvey)
			wndAlert:FindChild("CustomerSurveyAlertBtn"):SetData(csCurSurvey)
			wndAlert:SetTooltip(String_GetWeaselString(Apollo.GetString("CustomerSurvey_ClickToTake"), csCurSurvey:GetTitle()))
		end
	end
...
function CustomerSurvey:UpdateSurvey()
	...
	if self.csActiveSurvey and self.csActiveSurvey:GetSurveyId() ~= CustomerSurveyLib.CodeEnumCustomerSurvey.LogOff then
