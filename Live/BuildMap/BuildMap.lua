...
function BuildMap:DrawTierSettler(setNode)
	...
	for nTier, tCurrTier in ipairs(setNode:GetImprovements()) do
		if tCurrTier.idImprovement and not tCurrTier.bIsObsolete then
			...
			if tCurrTier.splDisplay then
				local tTooltips = tCurrTier.splDisplay:GetTooltips() 
				if tTooltips and tTooltips.strLASTooltip then
					strTooltip = string.format("<P Font=\"CRB_InterfaceMedium_O\">%s</P>%s", tTooltips.strLASTooltip, strTooltip)
				end
			end
