		...
		for idx, tRewardData in pairs(queQuest:GetRewardData().arFixedRewards) do
			...
			elseif tRewardData.eType == Quest.Quest2RewardType_Money then
				if tRewardData.eCurrencyType == Money.CodeEnumCurrencyType.Credits then					
					...
					local monAmount = Money.new()
					monAmount:SetMoneyType(tRewardData.eCurrencyType)
					monAmount:SetAmount(tRewardData.nAmount)
					local tDenomInfo = monAmount:GetDenomInfo()
					if tDenomInfo ~= nil and tDenomInfo[1] ~= nil then
						wndReward:FindChild("ItemCantUse"):Show(false)
						local wndItemIcon = wndReward:FindChild("ItemIcon")
						local strSprite = "ClientSprites:Icon_ItemMisc_bag_0001"
						if tDenomInfo[1].strSprite and tDenomInfo[1].strSprite ~= "" then
							strSprite = tDenomInfo[1].strSprite
						end
						wndItemIcon:SetSprite(strSprite)
						wndItemIcon:SetTooltip(monAmount:GetMoneyString())
					end
				end
			elseif tRewardData.eType == Quest.Quest2RewardType_AccountCurrency then
				local monAmount = Money.new()
				monAmount:SetAccountCurrencyType(tRewardData.eAccountCurrencyType)
				monAmount:SetAmount(tRewardData.nAmount)
				local tDenomInfo = monAmount:GetDenomInfo()
				if tDenomInfo ~= nil and tDenomInfo[1] ~= nil then
					wndReward:FindChild("ItemCantUse"):Show(false)
